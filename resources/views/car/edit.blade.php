...
<body>
    <div class="form-container">
        ...
        <div class="graphic-side">
            ...
            <h3 class="graphic-title">Update Vehicle Details</h3>
            <p class="graphic-text">Modify the details of your car to keep your fleet information accurate and up-to-date.</p>
            <div class="graphic-image"></div>
        </div>
        
        <!-- Form Side -->
        <div class="form-side">
            <h2 class="form-title"><i class="fas fa-car"></i> Edit Car Details</h2>
            ...
            <form id="editCarForm" action="{{ route('car.update', $car->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="form-row">
                    <div class="form-col">
                        <div class="input-group">
                            ...
                            <input type="text" name="brand" id="brand" class="input-field" value="{{ old('brand', $car->brand) }}" required>
                        </div>
                    </div>
                    <div class="form-col">
                        <div class="input-group">
                            <label for="model" class="input-label"><i class="fas fa-tools"></i> Model</label>
                            <input type="text" name="model" id="model" class="input-field" value="{{ old('model', $car->model) }}" required>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-row">
                    ...
                    <div class="form-col">
                        <div class="input-group">
                            ...
                            <input type="text" name="license_plate" id="license_plate" class="input-field" value="{{ old('license_plate', $car->license_plate) }}" required>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="form-col">
                        <div class="input-group">
                            ...
                            <select name="status" id="status" class="select-field" required>
                                <option value="good" {{ old('status', $car->status) == 'good' ? 'selected' : '' }}>Good</option>
                                <option value="bad" {{ old('status', $car->status) == 'bad' ? 'selected' : '' }}>Bad</option>
                                <option value="perfect" {{ old('status', $car->status) == 'perfect' ? 'selected' : '' }}>Perfect</option>
                    ...
                    <div class="form-col">
                        <div class="input-group">
                            ...
                            <select name="eco_friendly" id="eco_friendly" class="select-field" required>
                                <option value="1" {{ old('eco_friendly', $car->eco_friendly) ? 'selected' : '' }}>Yes</option>
                                <option value="0" {{ !old('eco_friendly', $car->eco_friendly) ? 'selected' : '' }}>No</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-col">
                        <div class="input-group">
                            ...
                            <input type="number" name="daily_rate" id="daily_rate" class="input-field" value="{{ old('daily_rate', $car->daily_rate) }}" required>
                        </div>
                    </div>
                    <div class="form-col">
                        <div class="input-group">
                    ...
                    <div class="form-col">
                        <div class="input-group">
                            ...
                            <select name="family_friendly" id="family_friendly" class="select-field">
                                <option value="1" {{ old('family_friendly', $car->family_friendly) ? 'selected' : '' }}>Yes</option>
                                <option value="0" {{ !old('family_friendly', $car->family_friendly) ? 'selected' : '' }}>No</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-col">
                        <div class="input-group">
                            ...
                            <input type="number" name="seats" id="seats" class="input-field" 
                                   value="{{ old('seats', $car->seats ?? 4) }}" min="1" max="12" required>
                        </div>
                    </div>
                </div>
                ...
                <div class="input-group">
                    ...
                    <div class="features-container">
                        <div style="padding: 17px ;margin-left: 20px;" class="feature-checkbox">
                            <input type="checkbox" name="child_seat" id="child_seat" value="1" {{ old('child_seat', $car->child_seat) ? 'checked' : '' }}>
                            <label for="child_seat">
                                <i class="fas fa-baby-carriage check-icon"></i> Child Seat
                            </label>
                        </div>
                        <div style="padding: 17px;margin-left: 20px;" class="feature-checkbox">
                            <input type="checkbox" name="air_conditioning" id="air_conditioning" value="1" {{ old('air_conditioning', $car->air_conditioning) ? 'checked' : '' }}>
                ...
                <div class="input-group">
                    ...
                    @if($car->picture)
                        <div class="mt-3">
                            <img src="/images/{{ $car->picture }}" alt="Current Car Image" class="car-image">
                ...
                <div class="form-row">
                    ...
                    <div class="form-col">
                        <button type="submit" id="submitBtn" class="update-btn"><i class="fas fa-save"></i> Update Car</button>
                    </div>
                </div>
            </form>
        </div>
            ...
            form.addEventListener('submit', function(e) {
                ...
                fetch('/api/check-subscription')
                    ...
                    .then(data => {
                        if (!data.is_subscribed) {
                            e.preventDefault();
                            alert('Your agency is not subscribed. Please subscribe to update car details.');
                            return false;
                        }
                    })
                    .catch(error => {
